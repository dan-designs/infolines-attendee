...
export default function PasswordResetScreen() {
  ...
  // Only Email is needed for Supabase Password Reset
  ...
  const handleReset = async () => {
    ...
    try {
      ...
      Alert.alert(
        ...
        [
          { text: 'OK', onPress: () => router.back() } // Route back to login
        ]
      );

    } catch (err: any) {
      console.error('Reset error:', err);
      // It's a best practice in security NOT to tell the user if the email exists or not
      // We show the same success message regardless, or a generic error if the network fails.
